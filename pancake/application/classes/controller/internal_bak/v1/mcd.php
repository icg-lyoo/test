...
class Controller_Internal_v1_Mcd extends Controller_Templates_InternalAPIv1
{
	...
	{
		$this->_config = Kohana::$config->load('internal/mcd');
	...
	{
		$unsaved_files = array();
		$profile_name = Arr::get($_REQUEST, 'profile', 'base_media');

		if(isset($_FILES['files']) && count($_FILES['files']) > 0 && isset($_POST['targets']) && count($_POST['targets']) > 0)
		{
//			if(empty($profile_name)) $profile_name = 'base';
