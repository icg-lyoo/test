...
class Controller_Patriots_v1_Zone extends Controller_Templates_APIv1
{
		...
		if (count($zone_id) > 0)
		{
			...
			if ($zone_type != 'menus')
			{
				...
				$insert_promo_rel = DB::insert('zones_content', array('zone_id', 'content_id'))
					->values(array($zone_id[0], $insert_promo[0]))
					->execute($this->client['database']);
				if (!$insert_promo_rel)
				{
					$this->client['database']->rollback();
	...
	public function action_add_menu()
	{
		...
		//Adds Menu
		$insert_menu = DB::insert('menus', array('title', 'type_detail'))
			->values(array($title, $type_detail))
			->execute($this->client['database']);
		if($insert_menu)
		{
	...
	public function action_remove_menu()
	{
        ...
        public function action_set_disclaimer()
        {
                ...
                $update = DB::update('themes')
                        ->set(array('disclaimer' => $disclaimer))
                        ->where('id', '=', $theme_id)
                        ->execute($this->client['database']);
        ...
        public function action_get_disclaimer()
        {
                ...
                $select = DB::select('disclaimer')
                        ->from('themes')
                        ->where('id', '=', $theme_id)
                        ->execute($this->client['database'])
			->as_array();
                
                $this->template->content = $select;
        }
}
