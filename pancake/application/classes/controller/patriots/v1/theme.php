...
class Controller_Patriots_v1_Theme extends Controller_Templates_APIv1
{
	...
	public function action_get()
	{
		...
		$results = DB::select('id', 'name', 'css_class', 'disclaimer')
			->from('themes')
			->where('id', '=', $id)
			->execute($this->client['database'])
			->as_array();
	...
	{
		$results = DB::select('id', 'name', 'css_class', 'parent_theme_id', 'disclaimer')
			->from('themes')
			->order_by('name', 'ASC')
			->execute($this->client['database'])
			->as_array();
...
//							->as_array();
                                                            ...
                                                            $all_menu_items = DB::select(array('mi.id', 'mi_id'), 'mi.title', 'mip.price', array('mmi.id', 'mmi_id'), 'mmi.sort', 'mi.description', 'mi.description_extra')
							->from(array('menus_items', 'mi'))
							->join(array('menus_menus_items', 'mmi'))
							->on('mi.id', '=', 'mmi.menu_item_id')
							->join(array('menus_items_prices', 'mip'))
